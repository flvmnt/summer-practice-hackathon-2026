...
| `respondToPromptAction` | `{ promptId, answer: 'yes'\|'no', sportPrefs?: string[], lat?, lng?, maxDistanceKm? }` | `{ ok, state: 'queued'\|'matched', group?: { id } }` — records the answer, then calls the same idempotent matching engine used by cron when `yes` |
